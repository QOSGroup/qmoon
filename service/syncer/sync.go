...
func NewSyncer(node *service.Node) *Syncer {
	return &Syncer{node: node, tmcli: lib.TendermintClient(node.BaseURL)}
}

// Block 同步块
func (s Syncer) Block(ctx context.Context) error {
	log.Printf("[Sync] Block %v ", s.node.ChanID)
...
// block
func (s Syncer) block(height int64) error {
...
func (s Syncer) tx(b *types.Block) error {
	cdc := lib.MakeCodec()
...
func parseTx(blockHeader types.BlockHeader, t qbasetypes.Tx, mt *models.Tx) error {
...
func parseITx(blockHeader types.BlockHeader, t qbasetxs.ITx, mt *models.Tx) error {
	cdc := lib.MakeCodec()
...
func (s Syncer) Validator(ctx context.Context) error {
	log.Printf("[Sync] Validator %v ", s.node.ChanID)
	...
	for {
		...
		case <-ctx.Done():
			return nil
		default:
			vals, err := s.node.GetQOSValidator(s.tmcli, 0, types.NodeType(s.node.NodeType))
			if err != nil {
				return err
			}
...
func (s Syncer) ConsensusState(ctx context.Context) error {
	log.Printf("[Sync] ConsensusState %v ", s.node.ChanID)
...
func (s Syncer) Peer(ctx context.Context) error {
	log.Printf("[Sync] Peer %v ", s.node.ChanID)
...
func (s Syncer) Lock(key string) bool {
...
func (s Syncer) Unlock(key string) bool {
	key = "lock_" + s.node.ChanID + "-" + key
